Below is the implementation of the backtest for the enhanced momentum trading strategy using deep learning. The implementation includes all necessary components, such as data preprocessing, model training, portfolio formation, and risk management. The code is structured to follow the instructions provided.
...
1. **Data Preprocessing**:
   - Normalizes monthly and daily returns using z-scores.
   - Adds an indicator variable for January.

2. **Model Training**:
   - Uses a custom deep learning model (RBM + FFNN) to predict class 2 probabilities.
   - Trains the model on historical data and uses it to predict future returns.

3. **Portfolio Formation**:
   - Ranks stocks based on predicted class 2 probabilities.
   - Takes long positions in the top decile and short positions in the bottom decile.

4. **Risk Management**:
   - Calculates position sizes based on risk percentage.
   - Implements stop-loss and take-profit levels.

5. **Optimization**:
   - Optimizes key parameters such as `n_months`, `n_days`, `top_decile`, `bottom_decile`, `risk_per_trade`, `stop_loss`, and `take_profit`.

6. **Moon Dev Themed Debug Prints**:
   - Includes emojis and themed messages for easier debugging.

---

### Expected Output:
- Initial backtest results with performance metrics (Sharpe ratio, annualized returns, etc.).
- Optimized parameters and improved performance metrics.
- Performance plots for both initial and optimized strategies.
