Below is the implementation of the backtest for the described strategy using Python and the `backtesting.py` framework. The implementation includes all necessary components, including data preprocessing, model training, portfolio formation, and risk management.
...
1. **Data Preprocessing**:
   - Computes cumulative returns for monthly and daily data.
   - Normalizes cumulative returns using z-scores.
   - Adds a January indicator.

2. **Model Training**:
   - Uses a stacked autoencoder with RBMs and an FFNN classifier.
   - Trains the model on historical data and evaluates its performance.

3. **Portfolio Formation**:
   - Ranks stocks based on predicted probabilities.
   - Opens long positions in the top decile and short positions in the bottom decile.

4. **Risk Management**:
   - Excludes stocks with prices below $5.
   - Equal weighting for all positions.

5. **Backtesting and Optimization**:
   - Runs a backtest with default parameters.
   - Optimizes key parameters like lookback periods and decile thresholds.

6. **Moon Dev Themed Debugging**:
   - Includes fun and informative debug prints 🌙 ✨ 🚀.
